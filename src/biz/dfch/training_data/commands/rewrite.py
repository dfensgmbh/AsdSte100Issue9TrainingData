...
def rewrite(
    ...
    file: NameArg = "task04.jsonl",
    overwrite: OverwriteArg = False,
):
    """
    Make the dataset for "Task 4: Sentence Rewriting for Compliance".

    Issue #4, https://github.com/dfensgmbh/AsdSte100Issue9TrainingData/issues/4

    Generates training examples that present a non-compliant sentence
    and require the model to rewrite it into an ASD-STE100 compliant
    form, substituting restricted words with their approved equivalents.
    ...
    log.debug(
        "rewrite: output=%s, file=%s, overwrite=%s", output, file, overwrite
    )
